...
let theme='dark';try{theme=localStorage.getItem(key)||'dark'}catch(e){}
...
const panel=document.querySelector('.settings-list');if(panel&&!document.getElementById('irisThemeToggle')){const b=document.createElement('button');b.id='irisThemeToggle';b.type='button';b.addEventListener('click',()=>{theme=theme==='light'?'dark':'light';try{localStorage.setItem(key,theme)}catch(e){}apply()});panel.insertBefore(b,panel.firstChild)}
apply();
})();